...
describe('static file handler (requires server at 8765)', () => {
  test('server is reachable (prerequisite)', async (t) => {
    const up = await isServerUp();
    if (!up) t.skip('Server not reachable at 127.0.0.1:8765');
    assert.ok(up, 'server is up');
  });
